...
function TaskItem({ task }: TaskItemProps) {
  ...
  return (
    <motion.div
      className="h-[16rem] px-4 py-3 flex flex-col gap-4 shadow-sm bg-[#dedede] rounded-lg border-2 border-stone-600"
      variants={item}
    >
      <div>
        <h4 className="font-bold text-2xl break-words">{task.title}</h4><br />
        <p>{task.description}</p>
        <h6 className="text-sm text-gray-400">Due: {formatTime(task.dueDate)}</h6>
      </div>
